...
if export_file and sms_file:
    # ---------------- Data Processing ----------------
    df1 = pd.read_excel(export_file, sheet_name=0)

    # Extract Purchasing Category
    ...
    # Plot 1: Monthly Distribution per Organization
    ...
    # Plot 2: Event Distribution per Cluster by Month (GPN only)
    ...
    # Plot 3: Event Distribution per Organization by Month (GPN only)
    if not df_gpn.empty:
    ...
    # Table 2: Pivot by Month/Organization
    ...
    # Table 3: Pivot by Month/Groups (GPN only)
    if not df_gpn.empty:
